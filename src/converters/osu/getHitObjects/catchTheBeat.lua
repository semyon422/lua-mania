...
local function getHitObjects(fileLines, first, last, cache)
	...
	for numberLine = first, last do
		...
		startX = raw[syntax.x]
		data.beatmap.objects.clean[startTime] = {type = type, startTime = startTime, endTime = endTime, startX = startX, endX = startX}
		table.insert(data.beatmap.hitSounds, {hitSound, volume})
